...
async function main() {
  ...
  const {g, eDict, e2v} = buildGraph(edges, vertices);
  ...
  function absBearingLeaving(e, vertex) {
    const [vx, vy] = vertex.coordinates;
    let deg;
    if (Math.abs(e.starting_point[0] - vx) < EPS && Math.abs(e.starting_point[1] - vy) < EPS) {
      deg = e.starting_point_direction;
    } else {
      deg = e.ending_point_direction + 180;
    }
    return (deg % 360 + 360) % 360;
  }

  function toClock(deg) {
  ...
  for (let i = 1; i < verts.length - 1; i++) {
    ...
    if (!vertex || vertex.connected_edges.length <= 2) continue;

    const inEid = routeEdgeIds[i - 1];
    const inBearing = absBearingLeaving(eDict[inEid], vertex);

    const connClocks = vertex.connected_edges
      .map(eid => {
        const absB = absBearingLeaving(eDict[eid], vertex);
        const rel = (absB - inBearing + 360) % 360;
        return { eid, clock: toClock(rel) };
      })
      .sort((a, b) => a.clock - b.clock);

    const outEid = routeEdgeIds[i];
    const nextInfo = connClocks.find(o => o.eid === outEid);

    branchInfos.push({
      vertex: vId,
      connections: connClocks.map(o => o.clock),
      next: nextInfo ? nextInfo.clock : null
    });
  }
